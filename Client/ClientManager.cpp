...
void ClientManager::setName(const QString &name)
{
    clientName = name;
}

void ClientManager::sendMessage(const QString &message, const QString &receiver)
{
    if (clientName.isEmpty()) {
        qWarning() << "Client name is not set.";
        return;
    }
    QByteArray formattedMessage = _protocol.textMessage(clientName, message, receiver);
